<div id="sidebar-menu">
    <ul class="metismenu list-unstyled" id="side-menu">
        ...
        @foreach ($akses_menu as $item)
            ...
            <?php $sub_menu = DB::table('submenu')->select('*')->where('menu_id',$item->id)->where('type_menu','view')->get(); ?>
                @if ($item->sub_menu > 0)
                    <li>
                            ...
                            @foreach ($sub_menu as $sub_item)
                                @if (in_array($sub_item->id, $session_submenu))
                                    @if ($sub_item->route_submenu)
                                        <li><a href="{{ route($sub_item->route_submenu) }}" class="waves-effect">{{ $sub_item->submenu }}</a></li>
                                    @else
