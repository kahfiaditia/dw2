<form action="{{ route('employee.update_sk') }}" enctype="multipart/form-data" onsubmit="return validate()"
    method="POST">
    @csrf
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="edit_jabatan" name="edit_jabatan"
                    value="{{ $item->jabatan }}" placeholder="Jabatan">
            </div>
        </div>
        <div class="col-md-6">
            <div class="mb-3">
                <input type="hidden" name="dok_sk_old" id="dok_sk_old" value="{{ $item->dok_sk }}">
                <label for="formFile" class="form-label">Dokumen SK (Max 2 Mb) <code>*</code></label>
    ...
    $(document).ready(function() {
        $('#edit_dok_sk').bind('change', function() {
            ...
            if (/\.(jpe?g|png|jpg|pdf)$/i.test(file.files[0].name) === false) {
                ...
                document.getElementById('edit_dok_sk').value = null;
            } else {
