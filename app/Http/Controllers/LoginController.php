...
class LoginController extends Controller
{
            ...
            if (Auth::user()->aktif === '1') {
                if (Auth::user()->roles == 'Administrator') {
                    ...
                    if ($setting->maintenance) {
                        ...
                        return back()->with('loginError', 'Maintenance Website');
                    } else {
                        $request->session()->regenerate();
                        return redirect()->intended('dashboard');
                    }
    ...
    public function verifikasi($id)
    {
        ...
        if (count($user) > 0) {
            ...
            return redirect('reverify')->with('Error', 'Reverify Email Fail!');
        }
    }
    ...
    public function reverifycode(Request $request)
    {
        ...
        if (count($cek) > 0) {
            if ($cek[0]->email_verified_at == null) {
                DB::beginTransaction();
                try {
                    $pin_verified = sprintf("%04d", rand(0, 9999));
                ...
                return redirect('reverify')->with('Error', 'Have Been Verified!');
            }
        } else {
